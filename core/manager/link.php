...
if(RQ_POST)
{
	if($action == 'addlink') 
	{
		...
		$url     = char_cv($url);
		$note    = char_cv($note);
		$rs = $DB->fetch_first("SELECT count(*) AS links FROM ".DB_PREFIX."link WHERE name='$name' AND url='$url'  ");
		if($rs['links'])
		{
			redirect('该链接在数据库中已存在', $admin_url.'?file=link');
		}
		$DB->query("INSERT INTO ".DB_PREFIX."link (name, url, note, visible,bak) VALUES ('$name', '$url', '$note' ,'$visible','$bak')");
		setting_recache();
	...
	else if($action=='domorelink')
	{
		...
		{
			$DB->query("DELETE FROM	".DB_PREFIX."link WHERE lid IN ($ids)  ");
		}
		if(is_array($_POST['name'])) 
		{
			foreach($_POST['name'] as $linkid => $value) 
			{
				$DB->unbuffered_query("UPDATE ".DB_PREFIX."link SET displayorder='".intval($_POST['displayorder'][$linkid])."', name='".char_cv(trim($_POST['name'][$linkid]))."', url='".char_cv(trim($_POST['url'][$linkid]))."', note='".char_cv(trim($_POST['note'][$linkid]))."', visible='".intval($_POST['visible'][$linkid])."',bak='".$_POST['bak'][$linkid]."' WHERE lid='".intval($linkid)."'  ");
			}
		}
		setting_recache();
...
if ($action == 'list')
 {
	$query = $DB->query("SELECT * FROM ".DB_PREFIX."link ORDER BY displayorder");
	$linkdb = array();
